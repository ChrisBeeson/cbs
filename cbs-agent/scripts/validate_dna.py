...
def validate(d):
    ...
    # minimal subject rule
    for grp in (d.get("interface",{}).get("subjects",{}).get("subscribe",[]) + d.get("interface",{}).get("subjects",{}).get("publish",[])):
        if not grp.startswith("cbs."):
            errors.append(f"bad subject (must start with 'cbs.'): {grp}")
    return errors
